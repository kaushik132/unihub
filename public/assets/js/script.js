...
var swiper = new Swiper(".eventsld", {
    slidesPerView: 2,
    spaceBetween: 10,
    loop: true,
    autoplay: {
...
///////////////////////swiper blogsld page banner//////////////////////////////
var swiper = new Swiper(".blogsld", {
    slidesPerView: 2,
    loop: true,
    spaceBetween: 20,
    autoplay: {
...
document.querySelectorAll('.has-submenu').forEach(item => {
    item.addEventListener('click', function(event) {
        ...
        // Get the submenu and the icon related to the clicked item
        const submenu = item.nextElementSibling;
        const icon = item.querySelector('.submenu-icon');

        // Check if the submenu is already active
        if (submenu.classList.contains('active')) {
            // If active, remove the 'active' class and rotate class to hide it and reset the icon
            submenu.classList.remove('active');
            icon.classList.remove('rotate');
        } else {
            // Close all sibling submenus within the same parent
            const parentMenu = item.parentElement.parentElement;
            parentMenu.querySelectorAll('.submenu').forEach(sub => {
                sub.classList.remove('active');
            });
            parentMenu.querySelectorAll('.submenu-icon').forEach(icn => {
                icn.classList.remove('rotate');
            });

            // Then add the 'active' class to the clicked submenu and rotate the icon
...
$('.burger-menu').click(function() {
    $('.mobile_navbar').addClass('open');
});

$('.close_btn').click(function() {
    $('.mobile_navbar').removeClass('open');

});
document.addEventListener('DOMContentLoaded', function() {
    // Show the popup after 10 seconds (or whatever time you specify)
    setTimeout(function() {
    ...
    document.getElementById('close_rs').addEventListener('click', function() {
...
document.addEventListener('DOMContentLoaded', function() {
    // Event listener for the button click to open the popup
...
$(document).ready(function() {
    ...
    // Trigger the counter when the section comes into view
    $(window).scroll(function() {
        if (isScrolledIntoView($('.countnn'))) {
            $('.countnn').counterUp({
                delay: 10,
                time: 2000
            });
            // Unbind the scroll event to prevent the counter from being triggered multiple times
            $(window).off('scroll');
        }
    });
});
...
document.getElementById("copy-button").addEventListener("click", function() {
    ...
    navigator.clipboard.writeText(textToCopy).then(() => {
        alert("Discount Coupon copied to clipboard!");
    }).catch(err => {
...
document.addEventListener('DOMContentLoaded', function() {
    // Event listener for the button click to open the popup
    document.getElementById('popupbou-open').addEventListener('click', function() {
        document.getElementById('popupbou').style.display = 'flex';
    });

    // Event listener for the close button inside the popup
    document.getElementById('popupbou-close').addEventListener('click', function() {
        document.getElementById('popupbou').style.display = 'none';
    });
});